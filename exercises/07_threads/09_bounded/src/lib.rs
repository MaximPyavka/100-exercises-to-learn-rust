...
use std::sync::mpsc::{Receiver, SyncSender, Sender, RecvError};
...
impl TicketStoreClient {
    pub fn insert(&self, draft: TicketDraft) -> Result<TicketId, RecvError> {
        ...
        let cmd = Command::Insert { draft, response_channel: resp_sender };
    ...
    pub fn get(&self, id: TicketId) -> Result<Option<Ticket>, RecvError> {
        let (resp_sender, resp_receiver) = std::sync::mpsc::channel();
        let cmd = Command::Get { id, response_channel: resp_sender };
